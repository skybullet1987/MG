...
SYMBOL_BLACKLIST = {
    "BTCUSDT",  # Reference symbol only — never trade (too expensive for small capital, always dust)
    "USDTUSDT", "USDCUSDT",
    "SHIBUSDT", "XMRUSDT", "ZECUSDT", "DASHUSDT",
    "LUNAUSDT", "LUNCUSDT", "USTCUSDT",
    # Forex pairs / stablecoins
    "EURUSDT", "GBPUSDT", "AUDUSD", "NZDUSD",
}
...
BYBIT_MIN_QTY_FALLBACK = {
    'ADAUSDT': 10.0, 'SOLUSDT': 0.05, 'DOTUSDT': 1.0, 'LINKUSDT': 0.5,
    'AVAXUSDT': 0.2, 'MATICUSDT': 10.0, 'ATOMUSDT': 0.5, 'NEARUSDT': 1.0,
    'LTCUSDT': 0.1, 'BCHUSDT': 0.1, 'ETCUSDT': 0.5, 'XRPUSDT': 2.0,
}
...
# Fee buffer applied when computing sell/cover quantity to prevent overselling.
# Applies to both long exits (sell) and short covers (buy-to-close) on Bybit Margin.
BYBIT_SELL_FEE_BUFFER = 0.002  # 0.2% (0.1% base fee + 0.1% safety margin)
...
def get_min_quantity(algo, symbol):
    ...
    try:
        ...
        pass
    if ticker in BYBIT_MIN_QTY_FALLBACK:
        return BYBIT_MIN_QTY_FALLBACK[ticker]
...
def smart_liquidate(algo, symbol, tag="Liquidate"):
    ...
    if price * abs(holding_qty) < min_notional * 0.9:
        return False
    # Verify fee reserve before closing (Bybit margin account requirement)
    ...
    if abs(holding_qty) < exit_min_qty:
        return False
    # The portfolio reflects the post-fee quantity on Bybit margin accounts.
    ...
    # errors on Bybit Margin (e.g. post-fee holdings are slightly less than the ordered quantity).  Apply a haircut before re-rounding to guarantee floor division
...
def cleanup_position(algo, symbol, record_pnl=False, exit_price=None):
    ...
    Supports both long and short positions via algo.position_directions.
    """
    ...
    if hasattr(algo, 'position_directions'):
        algo.position_directions.pop(symbol, None)
...
def sync_existing_positions(algo):
    ...
    for symbol in algo.Portfolio.Keys:
        ...
        if symbol not in algo.Securities:
            try:
                algo.AddCrypto(ticker, Resolution.Minute, Market.Bybit)
            except Exception as e:
                algo.Debug(f"Error adding crypto {ticker}: {e}")
                continue
        algo.entry_prices[symbol] = holding.AveragePrice
        # Infer direction from portfolio sign (negative quantity = short)
        direction = 1 if holding.Quantity > 0 else -1
        if hasattr(algo, 'position_directions'):
            algo.position_directions[symbol] = direction
        if direction == -1:
            if hasattr(algo, 'lowest_prices'):
        ...
        algo.Debug(f"SYNCED: {ticker} | {'SHORT' if direction == -1 else 'LONG'} | Entry: ${holding.AveragePrice:.4f} | Now: ${current_price:.4f} | PnL: {pnl_pct:+.2%}")
        if direction == 1 and current_price > holding.AveragePrice:
            algo.highest_prices[symbol] = current_price
        elif direction == -1 and current_price < holding.AveragePrice:
            if hasattr(algo, 'lowest_prices'):
                algo.lowest_prices[symbol] = current_price
...
def record_exit_pnl(algo, symbol, entry_price, exit_price):
    ...
    Supports both long and short positions via algo.position_directions[symbol]:
      direction =  1 → long  (profit when exit_price > entry_price)
      direction = -1 → short (profit when exit_price < entry_price)
    """
    if entry_price <= 0 or exit_price <= 0:
        algo.Debug(f"⚠️ Cannot record PnL for {symbol.Value}: invalid prices (entry=${entry_price:.4f}, exit=${exit_price:.4f})")
        return None

    direction = getattr(algo, 'position_directions', {}).get(symbol, 1)
...
def resync_holdings_full(algo):
    ...
    if missing:
        ...
        for symbol in missing:
            try:
                if symbol not in algo.Securities:
                    algo.AddCrypto(symbol.Value, Resolution.Minute, Market.Bybit)
                holding = algo.Portfolio[symbol]
...
def execute_buy(algo, symbol, quantity):
    """Place a Maker limit buy order at the best bid to capture 0.1% Maker fees (Bybit)."""
...
def execute_sell(algo, symbol, quantity):
    """Place a Maker limit sell order at the best ask to capture 0.1% Maker fees (Bybit)."""
    security = algo.Securities[symbol]
...
def execute_short(algo, symbol, quantity):
    ...
    a negative signed_qty for backtest compatibility. This function is available as an
    alternative for callers that want an explicit short-entry helper (e.g. execute_short
