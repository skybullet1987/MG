...
class RealisticCryptoSlippage:
    ...
    Calibrated against empirical Coinbase fill data.
...
def get_min_quantity(algo, symbol):
    ...
    try:
        ...
        pass
    if ticker in COINBASE_MIN_QTY_FALLBACK:
        return COINBASE_MIN_QTY_FALLBACK[ticker]
...
def smart_liquidate(algo, symbol, tag="Liquidate"):
    ...
    # errors on Coinbase Cash Modeling (e.g. post-fee BTC holdings are slightly less than
...
def sync_existing_positions(algo):
    ...
    for symbol in algo.Portfolio.Keys:
        ...
        if symbol not in algo.Securities:
            try:
                algo.AddCrypto(ticker, Resolution.Minute, Market.Coinbase)
            except Exception as e:
...
def resync_holdings_full(algo):
    ...
    if missing:
        ...
        for symbol in missing:
            try:
                if symbol not in algo.Securities:
                    algo.AddCrypto(symbol.Value, Resolution.Minute, Market.Coinbase)
                holding = algo.Portfolio[symbol]
