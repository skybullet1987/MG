...
class SimplifiedCryptoStrategy(QCAlgorithm):
    ...
    def Initialize(self):
        ...
        self.quick_take_profit = self._get_param("quick_take_profit", 0.015)
        self.tight_stop_loss   = self._get_param("tight_stop_loss",   0.012)
        self.atr_tp_mult  = self._get_param("atr_tp_mult",  4.0)
        self.atr_sl_mult  = self._get_param("atr_sl_mult",  2.0)
        self.trail_activation  = self._get_param("trail_activation",  0.012)
        self.trail_stop_pct    = self._get_param("trail_stop_pct",    0.008)
        ...
        self.coinbase_status = "unknown"
        self._last_skip_reason = None

        self.UniverseSettings.Resolution = Resolution.Minute
        self.AddUniverse(CryptoUniverse.Coinbase(self.UniverseFilter))
    ...
    def OnData(self, data):
        ...
        if not self._positions_synced:
            ...
            if self.coinbase_status == "unknown":
                self.coinbase_status = "online"
                self.Debug("Fallback: coinbase_status set to online after warmup")
    ...
    def Rebalance(self):
        ...
        if self.LiveMode and not live_safety_checks(self):
            return
        if self.LiveMode and getattr(self, 'coinbase_status', 'unknown') in ("maintenance", "cancel_only"):
            self._log_skip("coinbase not online")
            return
    ...
    def _execute_trades(self, candidates, threshold_now):
        if not self._positions_synced:
            return
        if self.LiveMode and self.coinbase_status in ("maintenance", "cancel_only"):
            return
        cancel_stale_new_orders(self)
        if len(self.Transactions.GetOpenOrders()) >= self.max_concurrent_open_orders:
            return
        ...
        for cand in candidates:
            ...
            try:
                ...
                if actual_min > 0 and qty < actual_min:
                    ...
                    continue
                if min_order_size > 0:
                    post_fee_qty = qty * (1.0 - COINBASE_SELL_FEE_BUFFER)
                    if post_fee_qty < min_order_size:
                        required_qty = round_quantity(self, sym, min_order_size / (1.0 - COINBASE_SELL_FEE_BUFFER))
        ...
        try:
            ...
            if "system status:" in txt:
                if "online" in txt:
                    ...
                    self.coinbase_status = "maintenance"
                elif "cancel_only" in txt:
                    self.coinbase_status = "cancel_only"
                elif "post_only" in txt:
                    self.coinbase_status = "post_only"
                else:
                    self.coinbase_status = "unknown"
                self.Debug(f"Coinbase status update: {self.coinbase_status}")
